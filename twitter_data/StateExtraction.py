...
def location2state(location):
	places = geograpy.get_place_context(text=location)
	if len(places.countries)>0:
		if places.countries[len(places.countries)-1]=="United States":
			for p in places.regions:
				if p in states.keys():
					return states[p]
				elif p in states.values():
					return p
	return "NULL"

def location2state_v2(location):
	...
	for p in places:
		if p in states.keys():
			...
			return p
		elif p in city2state.keys():
			return city2state[p]
	return 'NULL'

numOfData = 5314980
data = open('all_data.json','r').readlines()
...
for i in range(numOfData):
	...
	ret = pattern.findall(loc)
	if (len(ret)>0 and ret[0] != ""):
		state = location2state_v2(ret[0])
		if state != 'NULL':
			output.write(date)
			output.write(msg)
			output.write('  "location": "'+state+'",\n')
			j += 1
